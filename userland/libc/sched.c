...
int clone(int (*fn)(void*), void* stack, int flags, void* arg, ...) {
    ...
    va_list ap;
    va_start(ap, arg);
    if (flags & (CLONE_PARENT_SETTID | CLONE_SETTLS | CLONE_CHILD_CLEARTID))
        parent_tid = va_arg(ap, pid_t*);
    if (flags & (CLONE_SETTLS | CLONE_CHILD_CLEARTID))
        tls = va_arg(ap, void*);
    ...
    return __syscall_return(
        __clone(fn, stack, flags, arg, parent_tid, child_tid, tls));
}
