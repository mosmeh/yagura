...
static void decode(const struct registers* regs, unsigned long* out_number,
    ...
    if (out_number)
        *out_number = (unsigned int)regs->ax;
    if (out_args) {
        out_args[0] = (unsigned int)regs->bx;
        out_args[1] = (unsigned int)regs->cx;
        out_args[2] = (unsigned int)regs->dx;
        out_args[3] = (unsigned int)regs->si;
        out_args[4] = (unsigned int)regs->di;
        out_args[5] = (unsigned int)regs->bp;
    }
}
