...
static struct mount* minix_mount(const char* source) {
    struct path* path FREE(path) = ASSERT(vfs_resolve_path(source, 0));
    if (IS_ERR(path))
    ...
    *mount = (struct minix_mount){
    ...
    size_t superblock_block = 1;
    {
        struct block* block FREE(block) =
            ASSERT(block_load(mount->dev, superblock_block, BLOCK_SIZE));
        if (IS_ERR(block))
    ...
    if (!imap)
        return ERR_PTR(-ENOMEM);
    int rc = block_read(mount->dev, imap, imap_size, imap_offset);
    if (IS_ERR(rc))
        return ERR_PTR(rc);
    ...
    if (!zmap)
        return ERR_PTR(-ENOMEM);
    rc = block_read(mount->dev, zmap, zmap_size, zmap_offset);
    if (IS_ERR(rc))
        return ERR_PTR(rc);
...
void minix_init(void) {
    SLAB_INIT(&inode_slab, "minix_inode", struct minix_inode);
