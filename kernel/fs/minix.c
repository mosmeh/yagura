...
static struct inode* get_inode(struct minix_mount* mount, ino_t ino) {
    ...
    struct minix_inode* inode = ASSERT(slab_alloc(&inode_slab));
    if (IS_ERR(inode))
...
static struct mount* minix_mount(const char* source) {
    struct path* path FREE(path) =
        ASSERT(vfs_resolve_path(BASE_CWD, source, 0));
    if (IS_ERR(path))
    ...
    *mount = (struct minix_mount){0};

    int rc = bcache_init(&mount->bcache, block_dev, BLOCK_SIZE);
    if (IS_ERR(rc))
        return ERR_PTR(rc);
    ...
    {
        struct block* block FREE(block) =
            ASSERT(bcache_get(&mount->bcache, superblock_block, VM_READ));
        if (IS_ERR(block))
            return ERR_CAST(block);

        struct minix_superblock* sb = block->data;
        switch (sb->s_magic) {
    ...
    size_t imap_offset = (superblock_block + 1) * BLOCK_SIZE;
    size_t imap_size = mount->imap_nblocks * BLOCK_SIZE;
    unsigned char* imap FREE(kfree) = kmalloc(imap_size);
    if (!imap)
        return ERR_PTR(-ENOMEM);
    rc = bcache_read(&mount->bcache, imap, imap_size, imap_offset);
    if (IS_ERR(rc))
        return ERR_PTR(rc);

    size_t zmap_offset = imap_offset + imap_size;
    size_t zmap_size = mount->zmap_nblocks * BLOCK_SIZE;
    unsigned char* zmap FREE(kfree) = kmalloc(zmap_size);
    if (!zmap)
        return ERR_PTR(-ENOMEM);
    rc = bcache_read(&mount->bcache, zmap, zmap_size, zmap_offset);
    if (IS_ERR(rc))
        return ERR_PTR(rc);

    struct inode* root FREE(inode) = ASSERT(get_inode(mount, MINIX_ROOT_INO));
    if (IS_ERR(root))
...
void minix_init(void) {
    SLAB_INIT_FOR_TYPE(&inode_slab, "minix_inode", struct minix_inode);
