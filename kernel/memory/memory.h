...
struct vm_region {
    ...
    size_t offset; // Offset in pages in the vobj
    uint32_t flags;
    struct vobj* vobj;
    struct vm_region* prev;
    struct vm_region* next;
    struct vm_region* shared_next; // Next region sharing the same vobj
};
...
// Sets the vobj of the region. The region should not have a vobj.
void vm_region_set_vobj(struct vm_region*, struct vobj*);
...
NODISCARD int vm_region_set_flags(struct vm_region*, size_t offset,
                                  size_t npages, uint32_t flags, uint32_t mask);
...
static inline uint16_t vm_flags_to_pte_flags(uint32_t vm_flags) {
