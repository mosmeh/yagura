...
struct page_directory* page_directory_create(void) {
    ...
    for (uintptr_t virt_addr = KERNEL_VIRT_ADDR; virt_addr < KERNEL_HEAP_END;
...
void page_directory_destroy(struct page_directory* pd) {
    ...
    for (size_t i = 0; i < KERNEL_PDE_IDX; ++i) {
        if (pd->entries[i] & PTE_PRESENT)
            page_free_raw(pd->entries[i] / PAGE_SIZE);
    }
...
static struct kmap_ctrl* current_kmap_ctrl(void) {
    static struct kmap_ctrl kernel_kmap;
    return current ? &current->kmap : &kernel_kmap;
}
...
void* kmap(uintptr_t phys_addr) {
    ...
    struct kmap_ctrl* kmap = current_kmap_ctrl();
...
void kmap_switch(struct kmap_ctrl* kmap) {
    ...
    for (; i < kmap->num_mapped; ++i) {
        ...
        ASSERT(pte);
        *pte = kmap->phys_addrs[i] | PTE_WRITE | PTE_PRESENT;
