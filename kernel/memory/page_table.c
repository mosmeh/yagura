...
get_or_create_page_table(uintptr_t virt_addr) {
    ...
    if (!(*pde & PTE_PRESENT)) {
        ...
        if (IS_ERR(pfn))
            return ERR_PTR(pfn);
        *pde = (pfn * PAGE_SIZE) | PTE_WRITE | PTE_USER | PTE_PRESENT;
        created = true;
    }
...
NODISCARD static int map(uintptr_t virt_addr, size_t pfn, uint16_t flags) {
    ...
    if (IS_ERR(pte))
        return PTR_ERR(pte);
    *pte = (pfn * PAGE_SIZE) | flags | PTE_PRESENT;
    return 0;
}
...
int page_table_map(uintptr_t virt_addr, size_t pfn, size_t npages,
    ...
    for (size_t i = 0; i < npages; ++i) {
        int rc = map(virt_addr + i * PAGE_SIZE, pfn + i, flags);
        if (IS_ERR(rc))
            return rc;
    }
    flush_tlb_range(virt_addr, npages * PAGE_SIZE);
    return 0;
}
...
void* kmap_page(struct page* page) {
    ASSERT(page);
    return kmap(page_to_pfn(page) * PAGE_SIZE);
}
