...
get_or_create_page_table(uintptr_t virt_addr) {
    ...
    if (!(*pde & PTE_PRESENT)) {
        ...
        *pde = (phys_index * PAGE_SIZE) | PTE_WRITE | PTE_USER | PTE_PRESENT;
        created = true;
    }
...
struct page_directory* page_directory_create(void) {
    ...
    for (size_t virt_addr = KERNEL_IMAGE_END; virt_addr < KERNEL_HEAP_END;
...
void page_directory_destroy(struct page_directory* pd) {
    ...
    if (pd == current_page_directory()) {
        // current->vm->page_directory has to be updated BEFORE switching page
        // directory. Otherwise, when we are preempted between the two
        // operations, current->vm->page_directory will be out of sync with the
        // actual active page directory.
    ...
    for (size_t i = 0; i < KERNEL_PDE_IDX; ++i) {
        if (pd->entries[i] & PTE_PRESENT)
            page_free_raw(pd->entries[i] & ~PTE_FLAGS_MASK);
    }
...
static int map(uintptr_t virt_index, size_t phys_index, uint16_t flags) {
    volatile uint32_t* pte = get_or_create_pte(virt_index * PAGE_SIZE);
    if (IS_ERR(pte))
        return PTR_ERR(pte);
    *pte = (phys_index * PAGE_SIZE) | flags | PTE_PRESENT;
    return 0;
}

int page_table_map(size_t virt_index, size_t phys_index, size_t npages,
                   uint16_t flags) {
    for (size_t i = 0; i < npages; ++i) {
        int rc = map(virt_index + i, phys_index + i, flags);
        if (IS_ERR(rc))
            return rc;
    }
    flush_tlb_range(virt_index * PAGE_SIZE, npages * PAGE_SIZE);
    return 0;
}

int page_table_map_local(size_t virt_index, size_t phys_index, uint16_t flags) {
    int rc = map(virt_index, phys_index, flags);
    if (IS_ERR(rc))
        return rc;
    flush_tlb_single(virt_index * PAGE_SIZE);
    return 0;
}

static void unmap(size_t virt_index) {
    volatile uint32_t* pte = get_pte(virt_index * PAGE_SIZE);
    if (pte)
        *pte = 0;
}
...
void* kmap(uintptr_t phys_addr) {
    ...
    ASSERT(pte);
    *pte = phys_addr | PTE_WRITE | PTE_GLOBAL | PTE_PRESENT;
...
void* kmap_page(struct page* page) {
    ASSERT(page);
    return kmap(page_to_phys_index(page) * PAGE_SIZE);
}
...
void kmap_switch(struct kmap_ctrl* kmap) {
    ...
    for (; i < kmap->num_mapped; ++i) {
        ...
        ASSERT(pte);
        *pte = kmap->phys_addrs[i] | PTE_WRITE | PTE_GLOBAL | PTE_PRESENT;
