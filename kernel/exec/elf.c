...
static int load_segments(struct vm* vm, const struct exec_image* image,
                         size_t load_bias, bool fixed, void** out_base_addr,
                         void** out_phdr_addr) {
    const Elf32_Ehdr* ehdr = (const void*)image->data;
    const Elf32_Phdr* phdr = (const void*)(image->data + ehdr->e_phoff);
...
NODISCARD static void* load_interp(struct loader* loader,
    ...
    void* base = NULL;
    rc = load_segments(loader->vm, image, 0, false, &base, NULL);
    if (IS_ERR(rc))
        return ERR_PTR(rc);

    loader->entry_point = (void*)(ehdr->e_entry + (uintptr_t)base);
    return base;
}
...
int elf_load(struct loader* loader) {
    ...
    rc = load_segments(loader->vm, &loader->image, exec_base, exec_fixed, NULL,
                       &phdr_addr);
    if (IS_ERR(rc))
        return rc;
    ...
    if (interp) {
        ...
        if (IS_ERR(rc))
            return rc;
        interp_base = load_interp(loader, &interp_image);
        exec_image_unload(&interp_image);
        if (IS_ERR(interp_base))
            return PTR_ERR(interp_base);
    }
