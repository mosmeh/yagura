...
static size_t num_enabled_cpus;
...
void smp_init(void) {
    ...
    size_t ap_stack_size = STACK_SIZE * (num_enabled_cpus - 1);
...
static_assert(AP_TRAMPOLINE_ADDR < 0x100000, "AP_TRAMPOLINE_ADDR too high");
static_assert(AP_TRAMPOLINE_ADDR % 0x1000 == 0,
              "AP_TRAMPOLINE_ADDR misaligned");
...
void smp_start(void) {
    ...
    kernel_page_directory->entries[0].raw = 0;
    flush_tlb();
