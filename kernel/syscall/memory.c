...
static long process_vm_rw(pid_t pid, const struct iovec* user_local_iov,
    ...
    while (local_iov_index < liovcnt && remote_iov_index < riovcnt) {
        ...
        struct page* page = vm_get_page(vm, remote_iov.iov_base);
        if (IS_ERR(page))
            return PTR_ERR(page);
        if (!page)
            return -EFAULT;
        ...
        unsigned char* remote_addr = kaddr + page_offset;
        int rc = write
                     ? copy_from_user(remote_addr, local_iov.iov_base, to_copy)
                     : copy_to_user(local_iov.iov_base, remote_addr, to_copy);
        kunmap(kaddr);
        if (rc)
            return -EFAULT;
