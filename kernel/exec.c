...
static int execve(const char* pathname, struct string_vec* argv,
    ...
    exe = inode_map(path_into_inode(path), stat.st_size, VM_READ, 0);
    if (IS_ERR(exe)) {
        ret = PTR_ERR(exe);
        exe = NULL;
        goto fail_exe;
    ...
    struct vm_region* region = vm_alloc(vm, 2 + STACK_SIZE / PAGE_SIZE);
    if (IS_ERR(region)) {
        ret = PTR_ERR(region);
        goto fail_vm;
    }
    ...
    ret = vm_region_set_flags(region, 1, STACK_SIZE / PAGE_SIZE,
                              VM_READ | VM_WRITE | VM_USER, ~0);
    if (IS_ERR(ret))
        goto fail_vm;
